...
export const SelectManualCollectionStep: MergeWorkflowStep = (data) => {
  ...
  const manualCollections = data.in.attributes.filter((attribute) => {
    return attribute.data_source == DataSources.MANUAL_COLLECTION
  });
  ...
  const attributes = data.in.attributes.filter((attribute) => {
    return attribute.data_source == DataSources.MANUAL_COLLECTION || attribute.data_source == DataSources.MODIFICATION;
  });
